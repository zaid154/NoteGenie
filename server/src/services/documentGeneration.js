...
import {
  ...
  runTextPipeline,
  runFilePipeline,
} from "./generationOrchestrator.js";
...
export async function buildNotesFromText({ text, title, userId, body, onProgress }) {
  const { doc } = await runTextPipeline({ text, title, userId, body, onProgress });
  return doc;
}

export async function buildMaterialFromText(opts) {
  return runTextPipeline(opts);
}

export async function buildNotesFromFile({ buffer, originalname, mimetype, userId, body, onProgress }) {
  const { doc } = await runFilePipeline({ buffer, originalname, mimetype, userId, body, onProgress });
  return doc;
}

export async function buildMaterialFromFile(opts) {
  return runFilePipeline(opts);
}
